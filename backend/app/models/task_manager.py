...
# ── Definition layer (user-designed templates) ───────────────
...
class TaskTemplate(Base, TimestampMixin):
    ...
    project_tasks: Mapped[list["ProjectTask"]] = relationship(back_populates="template")
...
class ProjectTask(Base, TimestampMixin):
    ...
    template = relationship("TaskTemplate", back_populates="project_tasks")
    stages: Mapped[list["ProjectStage"]] = relationship(
        back_populates="project_task", cascade="all, delete-orphan", order_by="ProjectStage.order"
    )
...
class ProjectStage(Base, TimestampMixin):
    ...
    project_task = relationship("ProjectTask", back_populates="stages")
    slots: Mapped[list["ProjectSlot"]] = relationship(
        back_populates="stage", cascade="all, delete-orphan", order_by="ProjectSlot.id"
    )


class ProjectSlot(Base, TimestampMixin):
    ...
    stage = relationship("ProjectStage", back_populates="slots")
...
class SlotVersion(Base, TimestampMixin):
    ...
    slot = relationship("ProjectSlot", back_populates="versions")
    document = relationship("Document", foreign_keys=[document_id])
