...
### AI 智能助手
多供应商支持（DeepSeek / Qwen / OpenAI 兼容），对话式文档问答（RAG 向量检索 + Agent 工具调用），自动摘要生成，全文搜索毫秒级响应。AI 可自主搜索文档、查询库统计、获取文档详情。
...
```
      ...
      │               │               │               │               │
┌─────┴─────┐   ┌─────┴─────┐   ┌────┴─────┐   ┌─────┴────┐  ┌──────┴──────┐
│ PostgreSQL │   │   Redis   │   │  Celery  │   │  MinIO   │  │  AI Provider│
│ + pgvector │   │  Pub/Sub  │   │ Worker + │   │  :9000   │  │ DeepSeek /  │
│ + 全文搜索 │   │  + Cache  │   │   Beat   │   │          │  │ Qwen / OpenAI│
└───────────┘   └───────────┘   └──────────┘   └──────────┘  └─────────────┘
```
...
| **对象存储** | MinIO（S3 兼容） |
| **AI & RAG** | 多供应商 Function Calling + pgvector 向量检索 + fastembed |
| **前端框架** | Vue 3 + TypeScript + Vite 5 |
| **UI 组件** | Element Plus 2.6 + ECharts 5 |
...
├── backend/
│   ├── app/
│   │   ├── api/v1/           # 13 个 API 路由模块
│   │   ├── core/             # 缓存、安全、权限、分页、WebSocket
│   │   ├── models/           # 12 个 ORM 模型
│   │   ├── schemas/          # Pydantic 校验
│   │   ├── services/         # 25 个业务服务（含 RAG、Agent、AI 配置）
